  ...
  $.fn.binFilterProduct = function (opts) {
    ...
    $(document).ready(function () {
      ...
      $(".back-to-shop").click(function () {
        ...
        $("#price-filter-min-text", config.widget_id).val(
          $("#bwp_slider_price", config.widget_id).data("min")
        );
        $("#price-filter-max-text", config.widget_id).val(
          $("#bwp_slider_price", config.widget_id).data("max")
        );
    ...
    function _event_click_sub_categories() {
      ...
      $("li", $subcategories).click(function () {
        ...
        if ($(".filter_category_product").length > 0) {
          $(".item-category", ".filter_category_product").removeClass("active");
          $(
            ".item-category[data-id_category=" +
    ...
    function _event_click_categories() {
      ...
      $(".item-category", $subcategories).click(function () {
        ...
        if ($(".woocommerce-product-subcategories").length > 0) {
          $("li", ".woocommerce-product-subcategories").removeClass("active");
          $(
            "li[data-id_category=" + $id_category + "]",
    ...
    function _event_click_load_more() {
      $("nav.woocommerce-pagination .woocommerce-load-more").click(function (
        e
      ) {
        $(this).addClass("active");
        e.preventDefault();
    ...
    function _event_load_infinity() {
      $(window).scroll(function () {
        if (
          ...
          $(".woocommerce-load-more").length > 0
        ) {
    ...
    function _event_filter_product() {
      ...
      $("li", config.widget_id).on("click", function () {
        ...
        else $("input", $(this)).attr("checked", true);
        _eventFilter();
        return false;
      });
    ...
    function _eventFilter(
      ...
      if (load) {
        ...
        $(".content-products-list", ".main-archive-product").append(
          '<div class="loading loading-filter"></div>'
        );
      }
      ...
      if ($("li.active", ".woocommerce-product-subcategories").length > 0) {
        var $id_category = $(".woocommerce-product-subcategories")
          .find("li.active")
          .data("id_category");
      }
      if ($(".item-category.active", ".filter_category_product").length > 0) {
        var $id_category = $(".filter_category_product")
          .find(".item-category.active")
          .data("id_category");
      }
      if (back) {
        $id_category = config.id_category = 0;
      }
      $filter.orderby = $(".woocommerce-ordering")
        .find("li.active")
        .data("value");
      $filter.product_count = $(".woocommerce-sort-count")
        .find("li.active")
        .data("value");
      ...
      if (direction === false) {
        $filter.default_min_price = $(
          "#bwp_slider_price",
          config.widget_id
        ).data("min");
        $filter.default_max_price = $(
          "#bwp_slider_price",
          config.widget_id
        ).data("max");
      ...
      $filter.shop_paging = $(".woocommerce-pagination").data("shop_paging")
        ? $(".woocommerce-pagination").data("shop_paging")
        : "shop-pagination";
      jQuery.ajax({
        type: "POST",
        url: $ajax_url,
        ...
        success: function (result) {
          ...
          if (result.products) {
            if (loadmore) {
              $(
                ".content-products-list .products-list",
                ".main-archive-product"
              ).append(result.products);
            } else {
            ...
            _wpbingo_countdown_product(
              $("ul.products", ".main-archive-product")
            );
          ...
          /* CUSTOM CODE BY MARK DIONNIE BULINGTI GWF DEV*/
          /*Removed -300x768 on background image reference that causes low resolutions */
          function removeImageSize(text) {
            /*  Use a regular expression to match the pattern "digits x digits" and replace it with an empty string */
          ...
          if (direction) {
            if ($(".page-title").data("bg_default")) {
              if (result.result_background) {
                alert(result.result_background);
                $(".page-title").css(
                  "background-image",
                  "url(" + removeImageSize(result.result_background) + ")"
                );
              } else {
                $(".page-title").css(
                  "background-image",
                  "url(" + $(".page-title").data("bg_default") + ")"
                );
              }
            }
          }
          /* END OF CUSTOM CODE BY MARK DIONNIE BULINGTI GWF DEV*/
          _event_after_sucsess_ajax(result, config);
          if (load) {
            setTimeout(function () {
              $(".content-products-list", ".main-archive-product").removeClass(
                "active"
              );
      ...
      $(".quickview-button").on("click", function (e) {
        ...
        $.ajax({
          ...
          success: function (results) {
            ...
            if (
              typeof jQuery.fn.tawcvs_variation_swatches_form != "undefined"
            ) {
              $(".variations_form").wc_variation_form();
              $(".variations_form").tawcvs_variation_swatches_form();
            } else {
              var form_variation =
                $(".bwp-quick-view").find(".variations_form");
            ...
            if ($(".product-countdown", ".bwp-quick-view").length > 0) {
              _event_countdown_quickview(
                $(".product-countdown", ".bwp-quick-view")
              );
            }
    ...
    function _wpbingo_click_add_to_cart() {
      ...
      $(".cart-icon").on("click", function () {
        if (
          !$("body").hasClass("not-scroll") &&
          $(".petio-topcart").hasClass("popup")
        ) {
    ...
    function _event_load_slick_carousel($element) {
      $element.slick({
        ...
        verticalSwiping: $element.data("verticalswiping")
          ...
          : false,
        rtl:
          $("body").hasClass("rtl") && !$element.data("vertical")
            ? true
            : false,
    ...
    function _event_after_sucsess_ajax(result, config) {
      ...
      else $(".woocommerce-result-count").html("");
      if (result.total_html)
        $(".woocommerce-found-posts").replaceWith(result.total_html);
      else $(".woocommerce-found-posts").html("");
      ...
      if (result.base_url != "")
        history.pushState(
          {},
          "",
          result.base_url.replace(/&amp;/g, "&").replace(/%2C/g, ",")
        );
    }
    ...
    function _addClassProductList() {
      var class_product_default = $("ul.products-list").data("col")
        ? $("ul.products-list").data("col")
        : "";
      var class_product_item = $(".view-grid.active").data("col")
        ? $(".view-grid.active").data("col")
        : class_product_default;
      if (class_product_item) {
        ...
        if ($(".view-list").hasClass("active")) {
          ...
          $("ul.products-list li")
            .removeClass(class_product_item)
            .addClass(list_class);
        }
      }
    }
